export function Reality() {
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <div className="max-w-content mx-auto">
        <h2 className="text-2xl sm:text-3xl mb-8">
          E tu hai già abbastanza da gestire.
        </h2>
        <div className="text-lg text-brand-gray leading-relaxed space-y-6">
          <p>Gestire un'attività richiede tempo ed energia.</p>
          <p>Clienti da seguire, preventivi, fornitori, problemi quotidiani.</p>
          <p>
            Imparare anche come funziona Google, il sito web e la visibilità online diventa facilmente un secondo lavoro.
          </p>
          <p>
            Molti titolari sanno che potrebbero avere più clienti. Ma semplicemente non hanno il tempo per occuparsi di questa parte.
          </p>
        </div>
      </div>
    </section>
  );
}
